# 👋 ¡Hola! Soy Alejandra Perea

<div align="center">
  
### 📊 Analista de Datos en Marketing Digital | 📰 Periodista | 🛍️ Especialista en Retail

📈 Me encanta conectar los datos con los resultados de negocio.

[![LinkedIn](https://img.shields.io/badge/LinkedIn-0077B5?style=for-the-badge&logo=linkedin&logoColor=white)](https://www.linkedin.com/in//alejandra-perea-fernández-365b55183/)
[![Email](https://img.shields.io/badge/Email-D14836?style=for-the-badge&logo=gmail&logoColor=white)](alejandrapereaf@gmail.com)

</div>

---

## 🎯 Sobre Mí

Soy una profesional con **experiencia en el sector retail** y formación en Periodismo, especializada en **análisis de datos aplicado al marketing digital**. Mi combinación única de habilidades me permite entender tanto el comportamiento del cliente en tienda como traducir esos insights a estrategias digitales basadas en datos.

**Mi propuesta de valor:**
- 🔍 Capacidad para identificar patrones de comportamiento del consumidor desde mi experiencia directa en ventas
- 📈 Habilidades analíticas para convertir datos en estrategias accionables de marketing
- ✍️ Comunicación efectiva para presentar insights complejos de forma clara (background en periodismo)
- 💼 Conocimiento profundo del negocio y de nuestros clientes

---

## 🎓 Formación

**Máster en Análisis de Datos en Marketing Digital**  
*[Universidad de Alcalá] | [2025]*  


**Grado en Periodismo**  
*[Universidad Complutense de Madrid] | [2024]*

---

## 💼 Experiencia Profesional

### Vendedora | [Sfera/El Corte Inglés]
*[2019] - Actualidad*

Durante mi tiempo en tienda he desarrollado:
- 🎯 Comprensión profunda del customer journey y puntos de dolor del cliente
- 📊 Análisis de tendencias de ventas y comportamiento de compra
- 🤝 Habilidades de comunicación y presentación ante clientes
- 💡 Identificación de oportunidades de mejora en la experiencia de compra

**Esta experiencia me permite aportar una perspectiva única al análisis de datos:** entiendo los números porque conozco a las personas detrás de ellos.

### Au Pair | Frankfurt am Main (Alemania)
*[2015] - [2019|*

Experiencia internacional que desarrolló competencias clave:
- 🌍 Adaptabilidad en entornos multiculturales y capacidad de integración
- 🗣️ Dominio de idiomas extranjeros (alemán e inglés) en contexto profesional
- 🎯 Responsabilidad, autonomía y gestión del tiempo
- 🤝 Habilidades interpersonales y comunicación intercultural
- 💪 Resiliencia y capacidad de resolución de problemas

**Esta experiencia internacional me aporta:** una mentalidad global imprescindible para entender mercados diversos y audiencias internacionales en el entorno digital.

---

## 🌍 Idiomas

<div align="center">

| Idioma | Nivel | Contexto |
|--------|-------|----------|
| 🇪🇸 **Español** | Nativo | Lengua materna |
| 🇩🇪 **Alemán** | Profesional | Adquirido durante 3,5 años viviendo y trabajando en Alemania |
| 🇬🇧 **Inglés** | Profesional | Perfeccionado durante estancia en Alemania |

</div>

**Ventaja competitiva:** Capacidad para trabajar con equipos internacionales, analizar datos de mercados globales y adaptar estrategias de marketing a audiencias multilingües.

---

## 🚀 Proyectos Destacados

### 📈 Proyecto 1: Segmentación de clientes mediante K-Means

**Contexto:** Proyecto desarrollado en el Máster en Análisis de Datos en Marketing Digital, en el marco de la asignatura de *Segmentación y Clustering*.  

**Objetivo:** Aplicar técnicas de segmentación para identificar grupos de clientes con comportamientos y características similares, con el fin de optimizar la personalización de las estrategias de marketing.  

**Metodología:** Se implementó el algoritmo de **K-Means** para la agrupación de clientes, analizando variables clave de comportamiento y perfil demográfico. El proceso incluyó la estandarización de datos, la determinación del número óptimo de clústeres y la interpretación de los resultados obtenidos.  

**Resultados:** La segmentación permitió definir perfiles de cliente diferenciados, aportando información valiosa para la toma de decisiones estratégicas en campañas de marketing y fidelización.  



[📂 Ver proyecto completo →](https://nbviewer.org/github/alejandrapereaf/portfolio-analisisdedatos/blob/main/AlejandraPerea.PEC.Segmentaci%C3%B3n.ipynb)

---

### 🏆 Proyecto 2: **TFM: Medición y optimización del rendimiento de las estrategias de SEO aplicado a SICOR teleasistencia El Corte Inglés**
> **Trabajo Fin de Máster - Proyecto Principal**

**Descripción:** Auditoría SEO de la plataforma de SICOR teleasistencia.

**Herramientas utilizadas:**
- 🔍 **Análisis:** Screaming Frog, PageSpeed Insights, Sistrix, Brand24, Google Trends, etc.
- 📊 **Visualización:** Power BI (Cuadro de mandos interactivo)


**📊 [Ver Cuadro de Mandos SEO →](https://app.powerbi.com/links/8cvn34Y_UB?ctid=2b079dc7-e2ea-45bc-9182-0fde14b549b1&pbi_source=linkShare)**  
**📄 [Ver informe completo →](https://elcorteingles.sharepoint.com/:w:/r/sites/TFM-Grupo3/Documentos%20compartidos/TFM/TFM%20Final/Trabajo%20Fin%20de%20Master%20v0.7.docx?d=w2ac31fc34c1c4292a5661341f9605851&csf=1&web=1&e=Nzsdbc)**

---

### 🎵 Proyecto 3:Data Driving --> Análisis de Hábitos de Escucha Musical vs. Datos Meteorológicos
**Descripción:** Análisis exploratorio de datos personales de Spotify correlacionados con datos meteorológicos de la API de Meteostat para identificar patrones entre el clima y las preferencias musicales.

**Objetivos:**
- Explorar la relación entre condiciones climáticas y hábitos de consumo musical
- Identificar patrones de escucha según temperatura, precipitaciones y otras variables meteorológicas
- Aplicar técnicas de análisis de datos y visualización en Python

**Herramientas utilizadas:**
- 🐍 **Python:** pandas, numpy, matplotlib, seaborn
- 🎵 **Spotify API:** Extracción de datos personales de escucha
- 🌦️ **Meteostat API:** Datos meteorológicos históricos
- 📓 **Jupyter Notebook:** Documentación y análisis interactivo

**🔍 [Ver preprocesamiento de los datos en Jupyter Notebook →](https://github.com/alejandrapereaf/portfolio-analisisdedatos/blob/main/Preprocesamieto.ipynb)**
**📓 [Ver datos extraidos con API de Meteostat →](https://github.com/alejandrapereaf/portfolio-analisisdedatos/blob/main/2.%20Meteostat.ipynb)**  
**💻 [Ver informe completo en Jupyter Notebook →](https://github.com/alejandrapereaf/portfolio-analisisdedatos/blob/main/3.Informe.ipynb)**



---

## 🛠️ Habilidades Técnicas

### Análisis de Datos
![Python](https://img.shields.io/badge/Python-3776AB?style=for-the-badge&logo=python&logoColor=white)
![SQL](https://img.shields.io/badge/SQL-4479A1?style=for-the-badge&logo=mysql&logoColor=white)
![Excel](https://img.shields.io/badge/Excel-217346?style=for-the-badge&logo=microsoft-excel&logoColor=white)
![R](https://img.shields.io/badge/R-276DC3?style=for-the-badge&logo=r&logoColor=white)

### Visualización de Datos
![Tableau](https://img.shields.io/badge/Tableau-E97627?style=for-the-badge&logo=tableau&logoColor=white)
![Power BI](https://img.shields.io/badge/Power_BI-F2C811?style=for-the-badge&logo=powerbi&logoColor=black)
![Google Data Studio](https://img.shields.io/badge/Google_Data_Studio-4285F4?style=for-the-badge&logo=google&logoColor=white)

### Marketing Digital
![Google Analytics](https://img.shields.io/badge/Google_Analytics-E37400?style=for-the-badge&logo=google-analytics&logoColor=white)
![Google Ads](https://img.shields.io/badge/Google_Ads-4285F4?style=for-the-badge&logo=google-ads&logoColor=white)
![Meta Ads](https://img.shields.io/badge/Meta_Ads-0088CC?style=for-the-badge&logo=meta&logoColor=white)
![SEO](https://img.shields.io/badge/SEO-47A248?style=for-the-badge&logo=google&logoColor=white)

### Otras Herramientas
![Git](https://img.shields.io/badge/Git-F05032?style=for-the-badge&logo=git&logoColor=white)
![Jupyter](https://img.shields.io/badge/Jupyter-F37626?style=for-the-badge&logo=jupyter&logoColor=white)
![Google Sheets](https://img.shields.io/badge/Google_Sheets-34A853?style=for-the-badge&logo=google-sheets&logoColor=white)

---

## 📊 Tipos de Análisis que Puedo Realizar

- ✅ Análisis de comportamiento del cliente y segmentación
- ✅ Análisis de rendimiento de campañas digitales (SEM, Social Ads, Email)
- ✅ Modelado predictivo y forecasting
- ✅ Análisis de funnel de conversión y optimización
- ✅ Análisis de atribución multicanal
- ✅ Análisis de cohort y retención
- ✅ Dashboard y reportes automatizados
- ✅ A/B testing y análisis de experimentos
- ✅ Análisis de tendencias y estacionalidad
- ✅ Customer Lifetime Value (CLV) y análisis de rentabilidad


---

## 📫 Contacto

¿Quieres saber más sobre mi trabajo o discutir cómo puedo aportar valor al equipo de marketing digital?

- 📧 Email: [alejandrapereaf@gmail.com]
- 💼 LinkedIn: [Mi perfil](https://www.linkedin.com/in/alejandra-perea-fern%C3%A1ndez-365b55183/)

---

<div align="center">

### 💡 "Los datos cuentan historias. Mi trabajo es escucharlas y convertirlas en acción."

---

*Última actualización: [14/11/2025]*

</div>

